...
#'
#' # Example:
#' summarize_threeP(dataHeight,
#'   cvPercent = 50, trueNetVBAR = 10,
#'   height = T, plot = T
#' )
#'
#'
#' # 2. estimateVBAR is included (height data is not used, so is not included),
#'      data is plot-level.
#'
#'
...
#'
#' # Example:
#' summarize_threeP(dataEstimateVBAR,
#'   cvPercent = 50,
...
summarize_threeP <- function(data, cvPercent, trueNetVBAR,
  ...
  summary <- fullData %>%
    ...
    summarize(
      sampSize = length(plotNum), # number of plots
      ...
      lowerBoundCI = avgVol -
        qnorm(1 - (1 - desiredConfidence) / 2) * sqrt(avgVol / sampSize),
      upperBoundCI = avgVol +
        qnorm(1 - (1 - desiredConfidence) / 2) * sqrt(avgVol / sampSize)
    )
