...
summarize_poisson <- function(data, desiredConfidence = 0.95) {
  ...
  dataFrame <- data.frame(
    values = as.numeric(names(tableFreq)),
  ...
  summary <- dataFrame %>%
    ...
    summarize(
      ...
      lowerBoundCI = lambdaHat -
        qnorm(1 - (1 - desiredConfidence) / 2) * se,
      upperBoundCI = lambdaHat +
        qnorm(1 - (1 - desiredConfidence) / 2) * se
    )
  ...
  if (var(data) > overdispersionLimit) {
    warning(
      "Variance significantly higher than the mean. The data is influenced by overdispersion."
    )
  }
