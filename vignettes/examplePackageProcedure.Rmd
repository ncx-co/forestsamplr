...
The setting:
  ...
  She wanted to be smart about this. She'd expended a lot of effort collecting the data, managing, and storing it, being careful to have a truely random, unbiased dataset. She wanted to do analyze the data correctly. She also wanted to be transparent, and be sure that other statisticians would support her findings. They would have access to the easily repeatable R code that explicitely stated her steps of analysis - if they had any questions, it would be easy to prove her process. So, she decided to use the Forest Sampling R package.
...
First, she had to expand her data how she wanted and provide her attribute of interest. She wanted basal area per acre (given the incredibly rare, monstrously tall, secret population she sampled, this was a reasonable attribute to use). So, she ran this little R code:
...
She modified this to match her data's needs, ran the code, and got:
```{r include = TRUE}
summarize_all_srs(expanded, attribute = 'bapa', type = 'dataframe', popSize = NA, desiredConfidence = 0.90, infiniteReplacement = F)
```

The output she got is the calculated mean, variance, standard error, and the upper and lower limits of the 90% confidence interval.


She's pretty happy with this output. Then she wonders: what if this data was really from a systematic sample? She creates stratum information and manipulates her data to match a stratum dataset:
...
She looked in the Forest Sampling Vignette and figures out the call:

```{r include = TRUE}
summarize_stratified(stratumData, attribute = 'bapa', stratumTab, desiredConfidence = 0.90, post = T)
```

She feels accomplished - she now has summary statistics for her data! Congratulations, Wilma!
